...
template <typename T>
inline T schlick_fresnel(const T& F0, Real cos_theta, Real eta)
{
    Real cos2ThetaT = 1 - (1 - cos_theta * cos_theta) / eta / eta;
    if (cos2ThetaT > 0)
    {
        Real cosThetaT = std::sqrt(cos2ThetaT);
        return F0 + (Real(1) - F0) *
            pow(max(1 - cosThetaT, Real(0)), Real(5));
    }
    return 1;
}
...
inline Real D_GGX_aniso(Vector3 h, Real alphaX, Real alphaY)
{
    Real t = h.x * h.x / alphaX / alphaX + h.y * h.y / alphaY / alphaY + h.z * h.z;
    return 1.0 / (c_PI * alphaX * alphaY * t * t);
}
...
inline Real smith_masking_gtr2_aniso(const Vector3& v_local, Real alphaX, Real alphaY)
{
    Vector3 v2 = v_local * v_local;
...
inline Vector3 sample_visible_normals_aniso(const Vector3& local_dir_in, Real alphaX, Real alphaY, const Vector2& rnd_param)
{
    ...
    float lensq = hemi_dir_in.x * hemi_dir_in.x + hemi_dir_in.y * hemi_dir_in.y;
    auto tangent = Vector3(-hemi_dir_in.y / std::sqrt(lensq), hemi_dir_in.x / std::sqrt(lensq), Real(0));
    Vector3 T1 = lensq > 0 ? tangent : Vector3(1.0, 0.0, 0.0);
    ...
    Vector3 Nh = t1 * T1 + t2 * T2 + std::sqrt(std::max(Real(0), 1.0 - t1 * t1 - t2 * t2)) * hemi_dir_in;
    // Section 3.4: transforming the normal back to the ellipsoid configuration
    Vector3 Ne = normalize(Vector3(alphaX * Nh.x, alphaY * Nh.y, std::max(Real(0), Nh.z)));
    return Ne;
...
/// See "Sampling the GGX Distribution of Visible Normals", Heitz, 2018.
/// https://jcgt.org/published/0007/04/01/
inline Vector3 sample_DcosTheta(const Vector3& local_dir_in, Real alpha, const Vector2& rnd_param)
{
    // The incoming direction is in the "ellipsodial configuration" in Heitz's paper
    if (local_dir_in.z < 0)
    {
        // Ensure the input is on top of the surface.
        return -sample_DcosTheta(-local_dir_in, alpha, rnd_param);
    }
    Real cosTheta = 0, phi = c_TWOPI * rnd_param.y;
    
    cosTheta = std::sqrt((1.0 - rnd_param.x) / (1.0 + (alpha * alpha - 1.0) * rnd_param.x));
    Real sinTheta =  std::sqrt(std::max(0.0, 1.0 - cosTheta * cosTheta));
    //Real inner = (1 - std::pow(alpha * alpha, 1 - rnd_param.x)) / (1 - alpha * alpha);
    //Real cosTheta = std::sqrt(std::max(Real(0), inner));
    //Real sinTheta = std::sqrt(std::max(Real(0), 1 - inner));
    //Real phi = rnd_param.y * c_TWOPI;
    return normalize(Vector3{ sinTheta * std::cos(phi), sinTheta * std::sin(phi), cosTheta});
}


inline Real fresnel_R0(Real eta)
{
