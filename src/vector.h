...
template <typename T>
inline bool operator<(const TVector3<T>& u, const TVector3<T>& v)
{
    for (int i = 0; i < 3; i++)
    {
        if (u[i] != v[i])
...
template <typename T>
inline TVector3<T> lerp(const TVector3<T>& v0, const TVector3<T>& v1, float x)
{
    return TVector3<T>{
        v0[0] * (1 - x) + v1[0] * x,
            v0[1] * (1 - x) + v1[1] * x,
...
template <typename T>
inline TVector3<T> reflect(const TVector3<T>& I, const TVector3<T>& N)
{
    return I - N * dot(N, I) * Real(2);
}
...
template <typename T>
inline T exp(const TVector3<T>& v)
{
    return TVector3<T>{
        std::exp(v0[1]),
            std::exp(v0[1]),
...
template <typename T>
inline TVector3<T> minVec(const TVector3<T>& a, const TVector3<T>& b)
{
    return TVector3<T>{min(a.x, b.x), min(a.y, b.y), min(a.z, b.z) };
}

template <typename T>
inline TVector3<T> maxVec(const TVector3<T>& a, const TVector3<T>& b)
{
