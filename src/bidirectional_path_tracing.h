...
Spectrum transmission_homogenized(int medium_id,
    const Scene& scene,
    pcg32_state& rng,
    const Vector3& start,
    const Vector3& end,
    ...
    *p_trans_nee = make_const_spectrum(1.0);
    while (true)
    {
        ...
        if (shadow_medium != -1)
        {
            ...
            int channel = (int)(floor(next_pcg32_real<Real>(rng) * 3.0));
            int iteration = 0;
            Real accum_t = 0;
...
Spectrum sample_direct_lighting(const Scene& scene, pcg32_state& rng,
    ...
    Real* pdf_pos, Real* pdf_dir, PointAndNormal* point_on_light, int* lightId)
{
    ...
    *pdf_pos = pdf_pos_t * light_pmf(scene, light_id);
    ...
    Spectrum pdf_t;
    Spectrum T = transmission_homogenized(medium, scene, rng, rayOrigin, point_on_light->position, 0, &pdf_t);
    return E * G * T;
}

Spectrum path_sample_phase_function(int medium_id, const Ray& ray, const Scene& scene,
    pcg32_state& rng, Vector3* wout, Real* pdf, Real* pdfRev)
{
...
bool sample_vol_scattering_homogenized(int medium_id,
    const Ray& ray,
    pcg32_state& rng,
    Real tMax,
    Real* t_sample,
...
void random_walk(std::vector<PathVertex>& path, const Scene& scene, const Ray& initRay, const RayDifferential& rayDiff, 
    pcg32_state& rng,
    ...
    int maxDepth, TransportDirection direction)
{
    ...
    while (true)
    {
        ...
        if (currentMedium != -1)
        {
            ...
            scatter = sample_vol_scattering_homogenized(currentMedium, currentRay, rng, tMax,
            ...
            if (scatter)
            {
                ...
                Spectrum L = path_sample_phase_function(currentMedium, currentRay, scene, rng, &wout, &pdf_phase, &pdf_rev);
...
std::vector<PathVertex> gen_camera_subpath(const Scene& scene,
    ...
    int maxDepth)
{
    ...
    random_walk(path, scene, ray, ray_diff, rng, cameraVertex.beta, pdf_dir, maxDepth - 1, 
        TransportDirection::TO_LIGHT);
    return path;
}

std::vector<PathVertex> gen_light_subpath(const Scene& scene,
    pcg32_state& rng,
    int maxDepth)
{
    std::vector<PathVertex> path;
    ...
    if (record)
    {
        ...
        random_walk(path, scene, ray, ray_diff, rng, beta, record->pdf_dir, maxDepth - 1, TransportDirection::TO_VIEW);
    }
    return path;
}

Real MISWeight(const Scene& scene, const std::vector<PathVertex>& cameraPath,
...
Spectrum connect_path(const Scene& scene, pcg32_state& rng, const std::vector<PathVertex>& cameraPath,
    const std::vector<PathVertex>& lightPath, int s, int t, Vector2* sampleCoord)
{
    ...
    else if (t == 1)
    {
        ...
        if (pdf_camera != 0 && luminance(Wi) != 0)
        {
            ...
            *sampleCoord = pos;
            ...
            Spectrum pdf_t;
            L = qs.beta * f * Wi / pdf_camera * transmission_homogenized(cameraPath1[0].medium_id, scene, rng,
    ...
    else if (s == 1)
    {
        ...
        Real pdf_pos, pdf_dir;
        int light_id;
        Spectrum Le = sample_direct_lighting(scene, rng, pt, &pdf_pos, &pdf_dir, &point_on_light, &light_id);
    ...
    else
    {
        ...
        if (is_connectible(pt) && is_connectible(qs))
        {
            ...
            if (luminance(L) != 0)
            {
                L *= G * transmission_homogenized(get_medium(pt, scene, wout_C), scene, rng, pt.position, qs.position, 0, &pdf_t);
            }
        }
    }
    ...
    Real weight = MISWeight(scene, cameraPath1, lightPath1, s, t);
...
)
{
    ...
    std::vector<PathVertex> cameraPath = gen_camera_subpath(scene, screen_pos, rng, scene.options.max_depth + 2);
    std::vector<PathVertex> lightPath = gen_light_subpath(scene, rng, scene.options.max_depth + 1);
    Spectrum L = make_zero_spectrum();
    ...
    for (int t = 1; t <= cameraPath.size(); t++)
    {
        for (int s = 0; s <= lightPath.size(); s++)
        {
            ...
            // if (s != 1) continue;
            Vector2 rasterCoord;
            Spectrum Lpath = connect_path(scene, rng, cameraPath, lightPath, s, t, &rasterCoord);
