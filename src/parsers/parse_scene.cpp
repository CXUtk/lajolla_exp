...
Texture<Spectrum> parse_spectrum_texture(
    ...
    {
        ParsedTexture texture = parse_texture(node, default_map);
        return make_image_spectrum_texture(
            texture.filename.string(), texture.filename, texture_pool, texture.uscale, texture.vscale, texture.uoffset, texture.voffset);
    }
    else
    {
        printf("Error %s\n", type.c_str());
...
RenderOptions parse_integrator(pugi::xml_node node,
    ...
    {
        options.integrator = Integrator::PathWithFeatures;
        for (auto child : node.children())
        {
            std::string name = child.attribute("name").value();
            if (name == "maxDepth")
    ...
    {
        options.integrator = Integrator::BDPath;
        for (auto child : node.children())
        {
            std::string name = child.attribute("name").value();
            if (name == "maxDepth" || name == "max_depth")
    ...
    {
        options.integrator = Integrator::SPPM;
        for (auto child : node.children())
        {
            std::string name = child.attribute("name").value();
            if (name == "maxDepth" || name == "max_depth")
            ...
            else if (name == "photonsPerIteration" || name == "photons")
            {
                options.photonsPerIteration = parse_integer(
                    child.attribute("value").value(), default_map);
            }
            else if (name == "iteration")
            {
                options.iterations = parse_integer(
                    child.attribute("value").value(), default_map);
            }
        }
    }
    ...
    {
        options.integrator = Integrator::VCM;
        for (auto child : node.children())
        {
            std::string name = child.attribute("name").value();
            if (name == "maxDepth" || name == "max_depth")
    ...
    {
        options.integrator = Integrator::PSSMLT;
        for (auto child : node.children())
        {
            std::string name = child.attribute("name").value();
            if (name == "maxDepth" || name == "max_depth")
            ...
            else if (name == "mutations_per_pixel" || name == "mutations")
            {
                options.mutations_per_pixel = parse_integer(
                    child.attribute("value").value(), default_map);
            }
            else if (name == "bootstraps_per_pixel" || name == "bootstraps")
            {
                options.bootstrap_samples_per_pixel = parse_integer(
                    child.attribute("value").value(), default_map);
            }
        }
    }
    ...
    {
        options.integrator = Integrator::GDPT;
        for (auto child : node.children())
        {
            std::string name = child.attribute("name").value();
            if (name == "maxDepth" || name == "max_depth")
    ...
    {
        options.integrator = Integrator::RESTIRPT;
        for (auto child : node.children())
        {
            std::string name = child.attribute("name").value();
            if (name == "maxDepth" || name == "max_depth")
