...
struct RenderOptions {
    ...
    ShiftMappingType shiftMapping = ShiftMappingType::VertexReconnect;
    ...
    int iterations = 4;
    int mutations_per_pixel = 4;
    int bootstrap_samples_per_pixel = 4;
};
...
inline Real get_shadow_epsilon(const Scene &scene) {
    return min(scene.bounds.radius * Real(1e-5), Real(0.001));
}
