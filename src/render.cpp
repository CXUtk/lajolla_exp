            ...
            for (int y = y0; y < y1; y++)
            {
                for (int x = x0; x < x1; x++)
                {
                    ...
                    for (int s = 0; s < spp; s++)
                    {
                        ...
                        {
                            sampler.replay();
                            Gx_l += 0.5 * (L - vol_path_tracing(scene, x - 1, y, sampler));
                        }
                        if (x + 1 < w)
                        {
                            sampler.replay();
                            Gx_r += 0.5 * (vol_path_tracing(scene, x + 1, y, sampler) - L);
                        }
                        if (y - 1 >= 0)
                        {
                            sampler.replay();
                            Gy_u += 0.5 * (L - vol_path_tracing(scene, x, y - 1, sampler));
                        }
                        if (y + 1 < h)
                        {
                            sampler.replay();
                            Gy_d += 0.5 * (vol_path_tracing(scene, x, y + 1, sampler) - L);
                        }
                    }
                    {
    ...
    for (int I = 0; I < 100; I++)
    {
        for (int y = 0; y < h; y++)
        {
            for (int x = 0; x < w; x++)
            {
                ...
                if (x - 1 >= 0)
                {
                    v += img(x - 1, y) + img_Gx(x - 1, y);
                }
                if (x + 1 < w)
                {
                    v += img(x + 1, y) + img_Gx(x, y);
                }
                if (y - 1 >= 0)
                {
                    v += img(x, y - 1) + img_Gy(x, y - 1);
                }
                if (y + 1 < h)
                {
                    v += img(x, y + 1) + img_Gy(x, y);
                }
                img_copy(x, y) = v * 0.25;
            }
        }
