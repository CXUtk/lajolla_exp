    ...
    for (int num_vertices = 3; max_depth == -1 || num_vertices <= max_depth + 1; num_vertices++) {
        ...
        // So we only need to compute G(v_{i}, v_{i+1}) * f(v_{i-1}, v_{i}, v_{i+1}) * L(v_{i}, v_{i+1})
        {
            ...
            if (!is_envmap(light)) {
                ...
                Vector3 rayOrigin = vertex.position + N * get_shadow_epsilon(scene);
        ...
        Vector3 N = dot(dir_bsdf, vertex.geometric_normal) > 0 ? vertex.geometric_normal : -vertex.geometric_normal;
        Vector3 rayOrigin = vertex.position + N * get_intersection_epsilon(scene);
        Ray bsdf_ray{ rayOrigin, dir_bsdf, 0, infinity<Real>()};
        ...
        //if ((f.x + f.y + f.z) / p2 > 9999999)
        //{
        //    Frame frame = vertex.shading_frame;
        //    if (dot(frame.n, dir_view) < 0)
        //    {
        //        frame = -frame;
        //    }
        //    Vector3 wi = to_local(frame, dir_view), wo = to_local(frame, dir_bsdf);
        //    Vector3 H = normalize(dir_view + dir_bsdf);

        //    printf("Value: %lf, wi = (%lf, %lf, %lf), wo = (%lf, %lf, %lf)\n", length(f / p2), wi.x, wi.y, wi.z, wo.x, wo.y, wo.z);
        //}
        if (p2 <= 0) {
...
/// Unidirectional path tracing
Spectrum path_tracing_with_features(const Scene& scene,
    int x, int y, /* pixel coordinates */
    pcg32_state& rng, TraceFeature* feature)
{
    feature->componentType = ComponentType::Specular;
    ...
    feature->depth = dist;
    feature->normal = vertex.shading_frame.n;
    ...
    for (int num_vertices = 3; max_depth == -1 || num_vertices <= max_depth + 1; num_vertices++)
    {
        ...
        if (num_vertices == 3)
        {
            feature->componentType = bsdf_sample.sampleType;
            feature->albedo = bsdf_sample.albedo;
        }
        ...
        {
            px = pdf_sample_bsdf(mat, dir_view, dir_bsdf, vertex, scene.texture_pool);
            dir_bsdf = dir_bsdf - vertex.geometric_normal * 2.0 * dot(dir_bsdf, vertex.geometric_normal);
        }
        ...
        Real p2 = pdf_sample_bsdf(mat, dir_view, dir_bsdf, vertex, scene.texture_pool) + px;
        //if ((f.x + f.y + f.z) / p2 > 9999999)
        //{
        //    Frame frame = vertex.shading_frame;
        //    if (dot(frame.n, dir_view) < 0)
